...
class Jupiter():
    ...
    async def swap(
        ...
        if not instructions:
            url=self.ENDPOINT_APIS_URL['SWAP']
            transaction_data = httpx.post(url=url, json=transaction_parameters).json()
            return transaction_data['swapTransaction']
        else:
            url=self.ENDPOINT_APIS_URL['SWAP']+"-instructions"
            transaction_data = httpx.post(url=url, json=transaction_parameters).json()
